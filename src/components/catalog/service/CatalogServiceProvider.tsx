...
const CatalogServiceProvider: React.FC<CatalogServiceProviderProps> = ({
  ...
  const [extItemsMap, setExtItemsMap] = React.useState<{
    [uid: string]: CatalogItem[];
  }>({});
  const [extItemsErrorMap, setItemsErrorMap] = React.useState<{
    [uid: string]: Error;
  }>({});
  ...
  const loaded =
    ...
    (catalogProviderExtensions.length === 0 ||
      (showAlreadyLoadedItems
        ? catalogProviderExtensions.some(
            ({ uid }) => extItemsMap[uid] || extItemsErrorMap[uid],
          )
        : catalogProviderExtensions.every(
            ({ uid }) => extItemsMap[uid] || extItemsErrorMap[uid],
          )));
  ...
  const preCatalogItems = React.useMemo(() => {
    ...
    const itemMap = _.flatten(
      enabledCatalogProviderExtensions.map((e) =>
        catalogFilterExtensions
          ...
          .reduce(
            (acc, ext) => acc.filter(ext.properties.filter),
            extItemsMap[e.uid] ?? [],
          ),
      ),
    ).reduce((acc, item: CatalogItem) => {
      if (!item) return acc;
      acc[item.uid] = item;
      return acc;
    ...
    extItemsMap,
    loaded,
    enabledCatalogProviderExtensions,
    catalogFilterExtensions,
  ]);
  ...
  const failedCalls = catalogProviderExtensions.filter(
    ({ uid }) => extItemsErrorMap[uid],
  ).length;
