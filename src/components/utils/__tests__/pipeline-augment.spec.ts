...
describe('PipelineAugment test correct data is augmented', () => {
  it('expect additional resources to be correctly added using augmentRunsToData', () => {
    const newData = augmentRunsToData(
      testData[2].pipelines,
      testData[2].pipelineruns,
    );
    expect(newData.length).toBe(1);
    expect(newData[0].latestRun.metadata.name).toBe(
      testData[2].pipelineruns[0].metadata.name,
    );
  });

  it('expect additional resources to be added using latest run', () => {
    const newData = augmentRunsToData(
      testData[3].pipelines,
      testData[3].pipelineruns,
    );
    expect(newData.length).toBe(2);
    expect(newData[0].latestRun.metadata.name).toBe(
      testData[3].pipelineruns[1].metadata.name,
    );
...
describe('PipelineAugment test getRunStatusColor handles all ComputedStatus values', () => {
  it('expect all but PipelineNotStarted to produce a non-default result', () => {
    ...
    const defaultCase = getRunStatusColor(ComputedStatus[failCase], t);
    const allOtherStatuses = Object.keys(ComputedStatus)
      .filter(
        (status) =>
          status !== failCase &&
          ComputedStatus[status] !== ComputedStatus.Other,
      )
...
describe('PipelineAugment test correct task status state is pulled from pipeline/pipelineruns', () => {
  ...
  describe('Successfully completed Pipelines', () => {
    ...
    it('expect a complex pipeline to have task-count equal to Succeeded states', () => {
      const complexTestData =
        pipelineTestData[PipelineExampleNames.COMPLEX_PIPELINE];
  ...
  describe('Failed / Cancelled pipelines', () => {
    ...
    const sumCancelledTaskStatus = (status: TaskStatus): number =>
      status.Cancelled;
    const sumSuccededTaskStatus = (status: TaskStatus): number =>
      status.Succeeded;
    const complexTestData =
      pipelineTestData[PipelineExampleNames.COMPLEX_PIPELINE];
    ...
    it('expect a partial pipeline to have task-count equal to Failed and Cancelled states', () => {
      const partialTestData =
        pipelineTestData[PipelineExampleNames.PARTIAL_PIPELINE];
...
describe('Pipelinerun graph to show the executed pipeline structure', () => {
  ...
  it('expect to return null, if pipelinerun does not have pipeline labels or name in the metadata field', () => {
    const pipelineRun = _.omit(testPipelineRun, [
      'metadata.labels',
      'metadata.name',
    ]) as PipelineRunKind;
  ...
  it('expect to return pipeline, if pipelinerun has pipelineSpec in spec field', () => {
    const plrWithEmbeddedPipeline =
      pipelineTestData[PipelineExampleNames.EMBEDDED_PIPELINE_SPEC]
        .pipelineRuns[DataState.SUCCESS];
    const executedPipeline = getPipelineFromPipelineRun(
      plrWithEmbeddedPipeline,
    );
...
describe('getResourceModelFromBindingKind', () => {
  ...
  it('should be able to find TriggerBindingModel', () => {
    expect(getResourceModelFromBindingKind('TriggerBinding')).toBe(
      TriggerBindingModel,
    );
  });

  it('should return TriggerBindingModel for undefined', () => {
    expect(getResourceModelFromBindingKind(undefined)).toBe(
      TriggerBindingModel,
    );
  });
...
describe('PipelineAugment test correct task status state is shown when there is custom tasks', () => {
  it('should not count custom tasks in TaskStatus', () => {
    const customTaskPipeline =
      pipelineTestData[PipelineExampleNames.CUSTOM_TASK_PIPELINE];
    const taskStatus = getTaskStatus(
    ...
    const customTaskCount = totalPipelineRunCustomTasks(
      customTaskPipeline.pipeline,
    );
