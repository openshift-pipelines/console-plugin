import { detailsPage } from '../../commands/views/details-page';
...
export const pipelineDetailsPage = {
  verifyTitle: (pipelineName: string) =>
    detailsPage.titleShouldContain(pipelineName),

  verifyTriggerTemplateSection: () =>
    cy
      .get(pipelineDetailsPO.details.triggerTemplateSection)
      .should('be.visible'),

  verifyPage: () =>
    cy
      .get(pipelineDetailsPO.title)
      .should('contain.text', pageTitle.PipelineDetails),
  ...
  verifyFieldsInDetailsTab: () => {
    ...
    cy.get(pipelineDetailsPO.details.fieldNames.annotations).should(
      'be.visible',
    );
    ...
    cy.get(pipelineDetailsPO.details.fieldNames.tasks).should(
      'contain.text',
      'Tasks',
    );
  },
  ...
  selectTriggerTemplateLink: () =>
    cy.get(pipelineDetailsPO.details.triggerTemplateLink).click(),
  ...
  finallyNode: () => {
    return cy.get(
      '[data-test="pipeline-visualization"] [data-test="finally-node"]',
    );
  },
};
...
export const triggerTemplateDetailsPage = {
  verifyPage: () =>
    cy
      .get(triggerTemplateDetailsPO.title)
      .should('contain.text', pageTitle.TriggerTemplateDetails),
  verifyTabs: () => {
  ...
  verifyFields: () => {
    cy.byLegacyTestID('resource-summary').within(() => {
      cy.get(triggerTemplateDetailsPO.details.fieldNames.name).should(
        'be.visible',
      );
      cy.get(triggerTemplateDetailsPO.details.fieldNames.namespace).should(
        'be.visible',
      );
      cy.get(triggerTemplateDetailsPO.details.fieldNames.labels).should(
        'be.visible',
      );
      cy.get(triggerTemplateDetailsPO.details.fieldNames.annotations).should(
        'be.visible',
      );
      cy.get(triggerTemplateDetailsPO.details.fieldNames.createdAt).should(
        'be.visible',
      );
      cy.get(triggerTemplateDetailsPO.details.fieldNames.owner).should(
        'be.visible',
      );
  ...
  selectEventListener: () =>
    cy.get(triggerTemplateDetailsPO.details.eventListenerLink).click(),
};

export const eventListenerDetailsPage = {
  verifyPage: () =>
    cy
      .get(eventListenerDetailsPO.title)
      .should('contain.text', pageTitle.EventListenerDetails),
  verifyTabs: () => {
    cy.get(triggerTemplateDetailsPO.detailsTab).should('be.visible');
    cy.get(triggerTemplateDetailsPO.yamlTab).should('be.visible');
  },
  ...
  selectTriggerBindingLink: () =>
    cy.get(eventListenerDetailsPO.details.triggerBindingLink).click(),
};
