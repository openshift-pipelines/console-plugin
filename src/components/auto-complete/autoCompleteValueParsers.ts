...
import { AddTriggerFormValues, SelectedBuilderTask } from '../../types';
...
export const useBuilderParams = (
  ...
  const taskResultACs: string[] = computeAvailableResultACs(
    tasks,
    taskResources,
    taskIndex,
  );
...
export const useAddTriggerParams = (): string[] => {
  ...
  const bindingParamACs: string[] =
    triggerBinding?.resource?.spec?.params?.map(
      (param) => `tt.${paramToAutoComplete(param)}`,
    ) || [];
