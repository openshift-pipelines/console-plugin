...
import { safeYAMLToJS } from '../../../../../tests/utils/yaml';
...
import { navigateTo, sidePane } from '../../pages/app';
...
Then(
  'Tasks, Parameters, Resources and Workspaces sections are displayed',
  () => {
...
When(
  'user adds resource with name {string} to the {string}',
  (resourceName: string) => {
...
When(
  ...
  () => {
    pipelineBuilderPage.addParameters(
      'param-1',
      'description',
...
When(
  'user adds the image name to the pipeline task {string}',
  (pipelineTaskName: string) => {
    pipelineBuilderPage.clickOnTask(pipelineTaskName);
    cy.get(pipelineBuilderPO.formView.sidePane.imageName).type(
      'openshift/hello-openshift',
    );
    sidePane.close();
  },
...
When(
  'user adds the git url in the url Parameter in cluster task sidebar',
  () => {
...
Then(
  ...
  (workspaceName: string) => {
    cy.get(pipelineDetailsPO.details.fieldValues.workspace).should(
      'contain.text',
      workspaceName,
    );
  },
...
When('user selects the first task as {string}', (task: string) => {
...
When('user clicks on Add finally task again', () => {
  pipelineBuilderPage.addFinallyNode();
});

When('user selects {string} from finally task list', (finallyTask: string) => {
  pipelineBuilderPage.selectFinallyTask(finallyTask);
});
...
When('user navigates to When Expressions section', () => {
  cy.get(
    pipelineBuilderPO.formView.sidePane.addWhenExpression,
  ).scrollIntoView();
});

When('user clicks on Add When Expressions', () => {
  cy.get(pipelineBuilderPO.formView.sidePane.addWhenExpression).click();
});

Then(
  'user can see a diamond shaped structure appear in front of third task',
  () => {
...
Then(
  'user can see "Input", "Operator" and "Value" fields in When Expressions section',
  () => {
...
Then('user can see "Operator" has values "in" and "notin"', () => {
  cy.get(
    '[data-test="row formData.tasks.2.when.0"] [data-test~="operator"]',
  ).click();
...
Then(
  ...
  () => {
    cy.get(pipelineBuilderPO.formView.sidePane.whenExpression).within(() => {
      cy.get('[data-test="row formData.tasks.2.when.0"]').within(() => {
        cy.get('[data-test="values"] [data-test="add-action"]').should(
          'be.visible',
        );
...
Given('user has named pipeline as {string}', (pipelineName: string) => {
  pipelineBuilderPage.enterPipelineName(pipelineName);
});

Given(
  'user has tasks {string} and {string} in series',
...
Given('user has a finally task as {string}', (finallyTask: string) => {
  pipelineBuilderPage.addFinallyNode();
  pipelineBuilderPage.selectFinallyTask(finallyTask);
...
When('user clicks on Add When Expression', () => {
  cy.get(pipelineBuilderPO.formView.sidePane.addWhenExpression).click();
});

When('user enters the input value as {string}', (value: string) => {
  cy.get(
    '[data-test="when-expression"] [data-test="row formData.finallyTasks.0.when.0"]',
  ).as('whenExpression');
...
When(
  'user chooses the operator value {string} from the dropdown',
  (value: string) => {
    cy.get('@whenExpression').within(() => {
...
When('user enters the value as {string}', (value: string) => {
  cy.get('@whenExpression').within(() => {
...
When('user adds Name as {string}', (name: string) => {
  cy.get('@pipelineParameters')
    .get('[data-test="row formData.params.0"] [data-test="name"]')
    .type(name);
});

When('user Default value as {string}', (url: string) => {
  cy.get('@pipelineParameters')
...
When(
  'user clicks on Add Workspace and add name as {string}',
  (workspace: string) => {
    cy.get('[data-test="pipeline-workspaces"]').as('pipelineWorkspaces');
    cy.get('@pipelineWorkspaces').within(() => {
      cy.get('[data-test="add-action"]').click();
      cy.get('[data-test="row formData.workspaces.0"] [data-test="name"]').type(
        workspace,
      );
    });
  },
...
When('user clicks on {string} task node', (task: string) => {
...
Given('user is at YAML view of Pipeline Builder page', () => {
  navigateTo(devNavigationMenu.Pipelines);
  pipelinesPage.clickOnCreatePipeline();
  cy.get(pipelineBuilderPO.yamlView.switchToYAMLView).click();
});
...
When(
  'user enters the yaml content from yaml file {string}',
  (yamlFile: string) => {
    cy.fixture(
...
When('user enters yaml content from yaml file {string}', (yamlFile: string) => {
  cy.fixture(`pipelines-workspaces/sum-and-multiply-pipeline/${yamlFile}`).then(
    (yaml) => {
      yamlEditor.setEditorContent(yaml);
    },
  );
...
When(
  ...
  (taskName: string, publisher: string) => {
...
When(
  'user selects {string} from Add task quick search',
  (searchItem: string) => {
    cy.byTestID('task-list').click();
    cy.get(pipelineBuilderPO.formView.quickSearch).type(searchItem);
...
When(
  ...
  (taskName: string, provider: string) => {
    ...
    cy.get('.ocs-quick-search-list__item-content')
      // .contains('kn')
      .contains('TektonHub')
      .then(() => {
        cy.get(
          pipelineBuilderPO.formView.quickSearchListItem(
            taskName,
            provider,
            'TektonHub',
          ),
...
When(
  'user add array type parameter {string} value {string}',
...
Then('user see the added parameter value', () => {
  cy.get('#form-input-parameters-0-value-field').should(
    'have.value',
    'foo,bar,value1',
  );
});
