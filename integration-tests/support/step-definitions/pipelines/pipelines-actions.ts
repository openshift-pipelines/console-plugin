...
import { adminNavigationMenu } from '../../constants/static-text/global-text';
import {
  pipelineBuilderPO,
  pipelinesPO,
...
Then(
  'pipelines column Name display with value {string}',
  (pipelineName: string) => {
...
Then(
  'kebab menu contains options Start, Add Trigger, Edit Pipeline, Delete Pipeline',
  () => {
    pipelinesPage.verifyKebabMenu();
...
Then(
  'user is able to see Details, Metrics, YAML, Pipeline Runs and Parameters tabs',
  () => {
...
Then(
  'Actions menu display with options Start, Add Trigger, Edit Pipeline, Delete Pipeline',
  () => {
    cy.byTestActionID(pipelineActions.Start).should('be.visible');
    cy.byTestActionID(pipelineActions.AddTrigger).should('be.visible');
    cy.byTestActionID(pipelineActions.EditPipeline).should('be.visible');
...
Then(
  'Actions menu contains options Start, Add Trigger, Edit Pipeline, Delete Pipeline',
  () => {
    actionsDropdownMenu.clickActionMenu();
    cy.byTestActionID(pipelineActions.Start).should('be.visible');
    cy.byTestActionID(pipelineActions.AddTrigger).should('be.visible');
...
Given('pipeline run is available for {string}', (pipelineName: string) => {
  ...
  cy.get('[data-test="status-text"]', { timeout: 50000 }).should(
    'include.text',
    'Succeeded',
  );
  navigateTo(devNavigationMenu.Pipelines);
