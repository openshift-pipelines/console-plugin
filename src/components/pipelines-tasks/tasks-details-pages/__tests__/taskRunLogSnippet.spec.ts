...
} from '../../../logs/log-snippet-types';
import { getTRLogSnippet } from '../../taskRunLogSnippet';

const taskRunMock =
  pipelineTestData[PipelineExampleNames.WORKSPACE_PIPELINE].taskRuns[0];
const plrWithFailedTaskRun =
  pipelineTestData[PipelineExampleNames.BROKEN_MOCK_APP].pipelineRuns[
    DataState.FAILED1
  ];
...
describe('TaskRunLogSnippet test', () => {
  ...
  it('should return a Log message for failed TaskRun with task container', () => {
    ...
    expect(podName).toEqual(
      'broken-app-pipeline-j2nxzm-x-compile-8mq2h-pod-b9gsg',
    );
  });
