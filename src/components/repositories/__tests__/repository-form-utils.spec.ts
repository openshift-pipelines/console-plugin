...
const k8sListResourceItemsMock = k8sListItems as jest.Mock;
...
describe('createRepositoryResources', () => {
  ...
  it('creates repository CR and secret when token is provided', async () => {
    ...
    // 👇 Change ONLY this line to make it pass
    expect(k8sCreateMock).toHaveBeenNthCalledWith(2, {
      model: RepositoryModel,
      ns: 'test-ns',
      queryParams: {},
...
describe('detectGitType', () => {
  it('should return the correct git provider', () => {
    expect(detectGitType('https://github.com/example/nodejs-ex')).toEqual(
      GitProvider.GITHUB,
    );
    expect(detectGitType('https://gitlab.com/example/nodejs-ex')).toEqual(
      GitProvider.GITLAB,
    );
    expect(detectGitType('https://bitbucket.org/example/nodejs-ex')).toEqual(
      GitProvider.BITBUCKET,
    );
  });

  it('should return git provider as unsure type if it is self hosted', () => {
    expect(detectGitType('https://git.dev/example/nodejs-ex')).toEqual(
      GitProvider.UNSURE,
    );
    expect(detectGitType('https://dev/example/nodejs-ex')).toEqual(
      GitProvider.UNSURE,
    );
  });
  ...
  it('should return  invalid type', () => {
    ...
    expect(detectGitType('//dev/example/nodejs-ex')).toEqual(
      GitProvider.INVALID,
    );
  });
});
...
describe('recommendRepositoryName', () => {
  ...
  it('should recommend a repo cr name based on the url', () => {
    expect(recommendRepositoryName('https://example.com/myrepo')).toEqual(
      'git-myrepo',
    );
    expect(recommendRepositoryName('https://example.com/myRepo123')).toEqual(
      'git-my-repo-123',
    );
  });
});
...
describe('getPipelineRunDefaultTemplate', () => {
  ...
  it('should return custom template', async () => {
    ...
    expect(template).toEqual(
      expect.stringContaining('my-custom-template-string'),
    );
  });
});
