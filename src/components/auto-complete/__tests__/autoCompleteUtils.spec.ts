...
import { PipelineBuilderTaskResources, PipelineTask } from '../../../types';
import {
  paramToAutoComplete,
  taskToResult,
  taskToStatus,
...
describe('autoCompleteUtils', () => {
  describe('paramToAutoComplete', () => {
    ...
    it('should not be impacted by other properties in the Param', () => {
      expect(paramToAutoComplete({ name: 'test', type: 'string' })).toBe(
        'params.test',
      );
      expect(
        paramToAutoComplete({ name: 'test', type: 'string', default: '' }),
      ).toBe('params.test');
      expect(
        paramToAutoComplete({
          name: 'test',
      ...
      expect(
        paramToAutoComplete({ name: 'test', type: 'array', default: [''] }),
      ).toBe('params.test');
      expect(
        paramToAutoComplete({
          name: 'test',
  ...
  describe('workspaceToAutoComplete', () => {
    it('should return a valid workspace variable reference path', () => {
      expect(workspaceToAutoComplete({ name: 'workspace' })).toBe(
    ...
    it('should not be impacted by other properties in the Workspace', () => {
      expect(
        workspaceToAutoComplete({ name: 'workspace', optional: false }),
      ).toBe('workspaces.workspace.bound');
      expect(
        workspaceToAutoComplete({ name: 'workspace', optional: true }),
      ).toBe('workspaces.workspace.bound');
    });
  });
  ...
  describe('taskToStatus', () => {
    it('should return a valid task status variable reference path', () => {
      const pipelineTask: PipelineTask =
        pipelineTestData[PipelineExampleNames.WORKSPACE_PIPELINE].pipeline.spec
          .tasks[0];
  ...
  describe('taskToResult', () => {
    ...
    it('should return an array of match results', () => {
      ...
      expect(taskToResult(resources)(task)).toEqual([
