...
Given('pipeline {string} is executed for 5 times', (pipelineName: string) => {
  ...
  cy.get(pipelineRunDetailsPO.pipelineRunStatus).should(
    'not.have.text',
    'Running',
  );
...
When(
  'user enters Name as {string} in General section of Dockerfile page',
  (name: string) => {
...
When(
  'user selects {string} pipeline from the pipeline dropdown menu',
  (pipelineName: string) => {
...
When(
  'user clicks the Generate Webhook Secret to generate Webhook secret',
  () => {
