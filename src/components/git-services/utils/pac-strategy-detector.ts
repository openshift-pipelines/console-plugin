...
export const verifyPipelinesFile = async (
  file: string,
  gitService: BaseService,
) => {
  const trimmedFile = file?.startsWith('.tekton/')
    ? file?.replace('.tekton/', '')
    : file;

  const resourceContent = await gitService.getFileContent(
    `.tekton/${trimmedFile}`,
  );
...
export const isPacRepository = async (
  ...
  const isKindPipelineRunPresent = await isPipelineFilePresent(
    pacFiles,
    gitService,
  );
