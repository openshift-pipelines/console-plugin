...
export const repositoryDetailsPage = {
  ...
  verifyFieldsInDetailsTab: () => {
    ...
    cy.get(repositoryDetailsPO.details.fieldNames.namespace).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.labels).should('be.visible');
    cy.get(repositoryDetailsPO.details.fieldNames.annotations).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.createdAt).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.owner).should('be.visible');
    cy.get(repositoryDetailsPO.details.fieldNames.repository).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.userName).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.accessToken).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.webhookUrl).should(
      'be.visible',
    );
    cy.get(repositoryDetailsPO.details.fieldNames.webhookSecret).should(
      'be.visible',
    );
  },
