...
export const updateYAML = (path, data) =>
  path
    ? _.set(_.cloneDeep(yamlDataBasicPassState), path, data)
...
export const hasMultipleErrors =
  (expectedErrors: { yupPath: string; errorMessage: string }[]) => (error) => {
    expectedErrors.forEach((errorObj) =>
      hasError(errorObj.yupPath, errorObj.errorMessage)(error),
    );
  };
