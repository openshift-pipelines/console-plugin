...
> {
  ...
  readFile(file) {
    ...
    if (file.size > maxFileUploadSize) {
      this.props.onChange({
        errorMessage: t(
          'plugin__pipelines-console-plugin~Maximum file size exceeded. File limit is 4MB.',
        ),
      });
      return;
    }
  ...
  render() {
    ...
    return connectDropTarget(
      <div className="co-file-dropzone">
        {canDrop && (
          <div className={klass}>
            <p className="co-file-dropzone__drop-text">
              {t('plugin__pipelines-console-plugin~Drop file here')}
            </p>
          </div>
        )}
          ...
          <div className="modal-body__field">
            <div className="pf-v5-c-input-group">
              <input
                ...
                aria-describedby={
                  this.props.inputFieldHelpText ? `${id}-help` : undefined
                }
                readOnly
                disabled
              />
              <span className="pf-v5-c-button pf-m-tertiary co-btn-file">
                ...
                />
                {t('plugin__pipelines-console-plugin~Browse...')}
              </span>
            </div>
            ...
            {fileIsBinary && (
              <Alert
                ...
                variant="info"
                title={t(
                  'plugin__pipelines-console-plugin~Non-printable file detected.',
                )}
                data-test="alert-info"
              >
                {t(
                  'plugin__pipelines-console-plugin~File contains non-printable characters. Preview is not available.',
                )}
              </Alert>
            )}
          </div>
...
const FileInputComponent = DropTarget(
  NativeTypes.FILE,
  boxTarget,
  (connect, monitor) => ({
...
export const DroppableFileInput = withTranslation()(
  DroppableFileInputWithTranslation,
);
