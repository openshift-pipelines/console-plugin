...
import { modal } from '../../commands/views/modal';
import {
  pipelineActions,
  pipelineBuilderText,
  pipelineTabs,
...
import { devFilePage, gitPage } from '../../pages/git-page';
...
When(
  'user enters git url as {string} in start pipeline modal',
  (gitUrl: string) => {
...
Then(
  'user is able to see the pipelineRuns with status as {string}',
...
Given(
  'user is at the Pipeline Run Details page of pipeline {string}',
  (pipelineName: string) => {
    pipelinesPage.search(pipelineName);
    pipelinesPage.selectPipelineRun(pipelineName);
    pipelineRunDetailsPage.verifyTitle();
...
Then(
  ...
  (pipelineStatus: string) => {
    cy.get(pipelineRunsPO.pipelineRunsTable.status).should(
      'contain.text',
      pipelineStatus,
      {
        timeout: 10000,
...
Then(
  ...
  () => {
    cy.get(topologyPO.sidePane.resourcesTab.pipelineRuns).should(
      'not.be.greaterThan',
      3,
    );
  },
...
Then('View all link is displayed', () => {
  cy.get(topologyPO.sidePane.pipelineRunsLinks).should(
    'contain.text',
    'View all',
  );
});
...
Then(
  'Start Pipeline modal displays with Workspaces, Advanced Options sections',
  () => {
...
Then(
  'Actions menu contains options {string}, {string}',
  (option1: string, option2: string) => {
    actionsDropdownMenu.clickActionMenu();
    cy.byTestActionID(option1).should('be.visible');
    cy.byTestActionID(option2).should('be.visible');
...
When(
  ...
  (workspaceName: string) => {
    ...
    cy.get(pipelinesPO.startPipeline.emptyDirectoryInfo)
      ...
      .should(
        'contain.text',
        pipelineBuilderText.formView.startPipeline.EmptyDirectoryInfoMessage,
      );
...
When(
  ...
  (workspaceType: string) => {
    ...
    cy.selectByDropDownText(
      pipelinesPO.startPipeline.sharedWorkspace,
      workspaceType,
    );
  },
);
...
When(
  'user selects Storage Class as {string} from dropdown',
...
Then(
  ...
  (workspace: string) => {
    cy.get(
      pipelineRunDetailsPO.details.workspacesResources
        .volumeClaimTemplateResources,
    ).should('be.visible');
...
Then(
  'user will see VolumeClaimTemplate Workspace in Pipeline Run Details page',
  () => {
    cy.get(
      pipelineRunDetailsPO.details.workspacesResources
        .volumeClaimTemplateResources,
...
Then('user will see Empty Directory in Pipeline Run Details page', () => {
  cy.get(
    pipelineRunDetailsPO.details.workspacesResources.emptyDirectory,
  ).should('be.visible');
});
...
When(
  'user goes to failed pipeline run of pipeline {string}',
  (pipelineName: string) => {
    navigateTo(devNavigationMenu.Pipelines);
    pipelinesPage.search(pipelineName);
...
When('user opens pipeline run details', () => {
  cy.get('[aria-label="Breadcrumb"] li')
    .eq(1)
    .should('include.text', 'PipelineRun details');
});
...
Then('user can see Name and Value column under Pipeline Run results', () => {
  cy.get(pipelineRunsPO.pipelineRunsTable.resultTable).should(
    'contain.text',
    'Name',
  );
  cy.get(pipelineRunsPO.pipelineRunsTable.resultTable).should(
...
Given(
  'a node with an associated pipeline {string} is present',
  (pipelineName: string) => {
...
Then(
  'user is able to see No parameters are associated with this PipelineRun',
  () => {
...
Given(
  'pipeline run is displayed for {string} with parameters',
  (pipelineName: string) => {
    pipelinesPage.clickOnCreatePipeline();
...
When(
  'user is at the Pipeline Details page of pipeline {string}',
  (pipelineName: string) => {
    navigateTo(devNavigationMenu.Pipelines);
    pipelinesPage.search(pipelineName);
...
When(
  'user starts the pipeline {string} in Pipeline Details page',
  (pipelineName: string) => {
    pipelineDetailsPage.verifyTitle(pipelineName);
...
Given(
  'user has created a pipelineRun with sbom task {string}',
  async (pipelineRunName: string) => {
    cy.exec(
      `oc apply -f testData/sbom-pipelinerun/sbom-task.yaml -n ${Cypress.env(
        'NAMESPACE',
      )}`,
      {
        failOnNonZeroExit: false,
...
When(
  'user navigates to PipelineRun Details page {string}',
...
Then(
  'user can see Download SBOM and View SBOM section in PipelineRun details page',
  () => {
    pipelineRunDetailsPage.verifyTitle();
...
Given(
  'user has created a PipelineRun with scan task {string}',
  async (pipelineRunName: string) => {
    cy.exec(
      `oc apply -f testData/scan-pipelinerun/scan-task.yaml -n ${Cypress.env(
        'NAMESPACE',
      )}`,
      {
        failOnNonZeroExit: false,
...
Then(
  'user can see the vulnerabilities in the list page {string}',
  (pipelineRunName: string) => {
