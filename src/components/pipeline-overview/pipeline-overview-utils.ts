import { PIPELINE_RUN_AUTO_START_FAILED } from '../../consts';

export const getAllNotStartedPipelines = (): { [ns: string]: string[] } => {
  try {
    return JSON.parse(
      sessionStorage.getItem(PIPELINE_RUN_AUTO_START_FAILED) ?? '{}',
    );
  } catch (e) {
    return {};
  }
...
export const isPipelineNotStarted = (
  pipelineName: string,
  namespace: string,
): boolean => {
...
export const removePipelineNotStarted = (
  pipelineName: string,
  namespace: string,
): void => {
  if (!pipelineName || !namespace) return;

  const newList = getNotStartedPipelines(namespace).filter(
    (pName) => pName !== pipelineName,
  );
...
export const setPipelineNotStarted = (
  pipelineName: string,
  namespace: string,
): void => {
  if (!pipelineName || !namespace) return;
