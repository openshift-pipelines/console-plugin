...
const PipelineRunLogsComponent: React.FC<PipelineRunLogsProps> = ({
  ...
  const taskRunNames = getSortedTaskRun(tRuns, pipelineTasks)?.map(
    (tRun) => tRun.metadata.name,
  );
    ...
    <>
      {proxyUnavailable && (
        <Alert
          ...
          variant="warning"
          title={t(
            'Multi-cluster proxy service is not available. Unable to fetch TaskRuns from worker cluster.',
          )}
        />
      )}
      {pendingAdmission && (
