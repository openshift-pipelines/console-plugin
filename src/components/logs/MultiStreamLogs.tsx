...
import { LOG_SOURCE_WAITING } from '../../consts';
import { ScrollDirection, useScrollDirection } from './scroll';
...
export const MultiStreamLogs: React.FC<MultiStreamLogsProps> = ({
  ...
  const handleComplete = React.useCallback((containerName) => {
    ...
    const newRenderTo = dataRef.current.findIndex(
      (c, i) => completedRef.current[i] !== true,
    );
    if (newRenderTo === -1) {
  ...
  const autoScroll =
    ...
    scrollDirection !== ScrollDirection.scrolledToBottom;
