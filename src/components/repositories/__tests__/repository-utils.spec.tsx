...
describe('repository-util', () => {
  ...
  it('getGitProviderIcon should return Github icon', () => {
    const gitProviderIcon = getGitProviderIcon(
      'https://github.com/sclorg/ruby-ex.git',
    );
    expect(gitProviderIcon).toEqual(
      <GithubIcon title="https://github.com/sclorg/ruby-ex.git" />,
    );
  });
  ...
  it('getGitProviderIcon should return Gitlab icon', () => {
    ...
    expect(gitProviderIcon).toEqual(
      <GitlabIcon title="https://gitlab.com/gitlab-org/gitlab" />,
    );
  });

  it('getGitProviderIcon should return Bitbucket icon', () => {
    const gitProviderIcon = getGitProviderIcon(
      'https://bitbucket.org/vikram_raj/helloworld',
    );
    expect(gitProviderIcon).toEqual(
  ...
  it('getGitProviderIcon should return default git icon', () => {
    const gitProviderIcon = getGitProviderIcon(
      'https://githube.com/sclorg/ruby-ex.git',
    );
    expect(gitProviderIcon).toEqual(
      <GitAltIcon title="https://githube.com/sclorg/ruby-ex.git" />,
    );
  });
  ...
  it('getLabelValue should return correct label based on ref', () => {
    ...
    expect(label1).toBe('plugin__pipelines-console-plugin~Branch');
    const label2 = getLabelValue('refs-tags-cicd-demo');
    expect(label2).toBe('plugin__pipelines-console-plugin~Tag');
    const label3 = getLabelValue('refs/heads/main');
    expect(label3).toBe('plugin__pipelines-console-plugin~Branch');
    const label4 = getLabelValue('refs/tags/cicd/demo');
    expect(label4).toBe('plugin__pipelines-console-plugin~Tag');
  });
});
