...
import { yamlEditor } from '../../pages/app';
...
Given(
  'user created pipeline with workspace using yaml {string}',
  (yamlFileName: string) => {
    pipelineBuilderPage.clickYaml();
    yamlEditor.isLoaded();
...
When(
  'user fills the yaml editor with sample {string}',
  (yamlSample: string) => {
...
When(
  'user selects volume type {string} from workspaces dropdown',
  (volumeType: string) => {
...
Given(
  'user created pipeline {string} with workspace',
  (pipelineName: string) => {
    pipelinesPage.clickOnCreatePipeline();
    pipelineBuilderPage.createPipelineWithWorkspaces(pipelineName);
    cy.byLegacyTestID('breadcrumb-link-0').click();
...
When(
  'user selects {string} from Config Map dropdown',
...
When(
  'user is at PipelineRun Details Page of {string}',
