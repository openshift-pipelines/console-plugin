...
import { navigateTo, perspective, yamlEditor } from '../../pages/app';
import {
  actionsDropdownMenu,
  tableFunctions,
} from '../../pages/functions/common';
import { tasksPage } from '../../pages/pipelines/task-page';
import { editAnnotations } from '../../pages/topology-page';
    ...
    if ($body.find(pipelinesPO.repositoriesTab).length > 0) {
      cy.get(pipelinesPO.repositoriesTab).then(($header) => {
        if ($header.is(':visible')) {
          cy.get(pipelinesPO.repositoriesTab).should(
            'include.text',
            'Repositories',
          );
        }
...
When(
  'user creates repository using YAML editor from {string}',
  (yamlLocation: string) => {
...
When(
  'user clicks on the repository {string} on Repositories page',
  (repoName: string) => {
...
When(
  'user searches and selects the repository {string} on Repository page',
  (repoName: string) => {
    pipelinesPage.search(repoName);
    pipelinesPage.verifyPipelinesTableDisplay();
    cy.byLegacyTestID(repoName).click();
...
Given(
  'repository {string} is present on the Repositories page',
  (repoName: string) => {
    pipelinesPage.selectTab(pipelineTabs.Repositories);
    pipelinesPage.search(repoName);
  },
...
When(
  'user searches repository {string} in repositories page',
  (repoName: string) => {
    pipelinesPage.search(repoName);
  },
);
...
When(
  'user clicks on kebab menu of the repository {string}',
  (repoName: string) => {
...
Then(
  'label {string} should be present in the labels section',
  (label: string) => {
...
When(
  'user adds key {string} and value {string}',
...
Then(
  'annotation section contains the value {string}',
...
Given('pipeline run is displayed for {string}', (repoName: string) => {
  cy.exec(
...
When(
  'user clicks Last Run value of repository {string}',
  (repoName: string) => {
    pipelinesPage.search(repoName);
...
Then(
  'Details tab is displayed with fields Repository, Branch, Commit id and Event type',
  () => {
...
Then(
  'user can see {string}, {string} and {string}',
  (el1, el2, el3: string) => {
