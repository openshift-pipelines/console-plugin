...
Given(
  'user selected Add Trigger from kebab menu of pipeline {string}',
  (pipelineName: string) => {
    pipelinesPage.selectActionForPipeline(
      pipelineName,
      pipelineActions.AddTrigger,
    );
  },
);
...
Then(
  'user should be able to see {string} link with variables section',
  (linkName: string) => {
...
Given(
  'user is at pipeline Details page of pipeline {string}',
  (pipelineName: string) => {
    pipelinesPage.selectPipeline(pipelineName);
...
Given(
  'user is at Trigger Template Details page of pipeline {string}',
  (pipelineName: string) => {
    pipelinesPage.selectPipeline(pipelineName);
    pipelineDetailsPage.verifyPage();
    pipelineDetailsPage.selectTriggerTemplateLink();
...
Given(
  'user is at Event Listener Details page of pipeline {string}',
  (pipelineName: string) => {
    pipelinesPage.selectPipeline(pipelineName);
    pipelineDetailsPage.verifyPage();
    pipelineDetailsPage.selectTriggerTemplateLink();
...
Then(
  ...
  () => {
    cy.get(pipelinesPO.removeTrigger.triggerTemplate).should(
      'have.text',
      'Select TriggerTemplate',
    );
  },
);
...
When(
  'user enters git url as {string} in add trigger modal',
  (gitUrl: string) => {
...
When(
  'user enters revision as {string} in add trigger modal',
  (revision: string) => {
