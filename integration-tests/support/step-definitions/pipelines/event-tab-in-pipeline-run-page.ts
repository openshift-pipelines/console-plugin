...
import { app, perspective } from '../../pages/app';
...
When(
  'user clicks on pipeline run for pipeline {string}',
  (pipelineName: string) => {
...
When('task named {string} is available with task run', () => {
  ...
  pipelinesPage.setEditorContent(
    'testData/task-creation-with-yaml/create-pipeline-with-task.yaml',
  );
  cy.get(pipelineBuilderPO.create).click();
  actionsDropdownMenu.selectAction('Start');
  tasksPage.togglePipelineSidebar();
...
When(
  'user can see events streaming for task runs and all associated pods',
  () => {
    app.waitForLoad();
    cy.get('[role="rowgroup"]').should('be.visible');
  },
