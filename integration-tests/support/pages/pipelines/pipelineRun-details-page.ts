...
export const pipelineRunDetailsPage = {
  ...
  verifyPipelineRunStatus: (status: string) => {
    ...
    cy.get(pipelineRunDetailsPO.pipelineRunStatus, { timeout: 60000 }).then(
      (actualStatus) => {
        if (actualStatus.text().includes('Failed')) {
          cy.get('dt')
            ...
            .then((message) => {
              cy.log(
                `Pipeline Run status is failed with the following message: ${message.text()}`,
              );
            });
          cy.get('dt')
  ...
  verifyFields: () => {
    cy.byLegacyTestID('resource-summary').within(() => {
      ...
      cy.get(pipelineDetailsPO.details.fieldNames.namespace).should(
        'be.visible',
      );
      ...
      cy.get(pipelineDetailsPO.details.fieldNames.createdAt).should(
        'be.visible',
      );
  ...
  verifyPipelineRunColumns: () => {
    ...
    cy.get(pipelineRunDetailsPO.taskRuns.columnNames.commidID).should(
      'be.visible',
    );
    cy.get(pipelineRunDetailsPO.taskRuns.columnNames.status).should(
      'be.visible',
    );
    cy.get(pipelineRunDetailsPO.taskRuns.columnNames.taskStatus).should(
      'be.visible',
    );
    ...
    cy.get(pipelineRunDetailsPO.taskRuns.columnNames.branch).should(
      'be.visible',
    );
  },
  selectPipeline: () =>
    cy.get(pipelineRunDetailsPO.details.pipelineLink).click(),
...
export const pipelineRunsPage = {
  ...
  selectKebabMenu: (pipelineRunName: string) => {
    ...
    cy.get(pipelineRunsPO.pipelineRunsTable.pipelineRunName).each(
      ($el, index) => {
