...
Then(
  'user can see different task runs based on number of tasks executed in pipeline',
  () => {
...
Then('user can see Name, Task, Pod, Status and Started columns', () => {
  ...
  cy.get(pipelineRunDetailsPO.taskRuns.columnNames.started).should(
    'be.visible',
  );
});
  ...
  (pipelineName: string, workspaceType: string) => {
    ...
    switch (workspaceType) {
      ...
      case 'Config Map':
        ...
        startPipelineInPipelinesPage.selectConfigMap(
          'sensitive-recipe-storage',
        );
        break;
      case 'Secret':
        cy.exec(
...
Given(
  'user is at Task Runs tab of pipeline run with all kind of Workspaces',
  () => {
...
Then('user can see Details, Log, YAML and Events tab', () => {
  cy.get(pipelineRunDetailsPO.taskRunsDetails.columnNames.details).should(
    'be.visible',
  );
  cy.get(pipelineRunDetailsPO.taskRunsDetails.columnNames.logs).should(
    'be.visible',
  );
  cy.get(pipelineRunDetailsPO.taskRunsDetails.columnNames.YAML).should(
    'be.visible',
  );
  cy.get(pipelineRunDetailsPO.taskRunsDetails.columnNames.events).should(
    'be.visible',
  );
});
...
Then('user is redirected to Task Run Details tab', () => {
  cy.get('[aria-label="Breadcrumb"] li')
    .eq(1)
    .should('include.text', 'TaskRun details');
});
...
Given(
  'user is at pipeline details page with pipeline runs {string}',
  (pipelineName: string) => {
    detailsPage.titleShouldContain(pageTitle.Pipelines);
    pipelinesPage.selectPipeline(pipelineName);
    cy.get('[aria-label="Breadcrumb"] li')
      .eq(1)
      .should('include.text', 'Pipeline details');
  },
);
...
When(
  'user clicks on pipeline runs tab for pipeline {string}',
  (pipelineName: string) => {
    cy.get(`[data-test-id^="${pipelineName}"]`).click();
    pipelineDetailsPage.selectTab('Pipeline Runs');
  },
...
When('user disables Duration column', () => {
  cy.get(
    pipelineRunDetailsPO.taskRuns.manageColumns.durationCheckbox,
  ).uncheck();
  cy.get(pipelineRunDetailsPO.taskRuns.manageColumns.submitButton).click();
});

Then('user sees Duration column', () => {
  cy.get(pipelineRunDetailsPO.taskRuns.columnNames.duration).should(
    'be.visible',
  );
});
