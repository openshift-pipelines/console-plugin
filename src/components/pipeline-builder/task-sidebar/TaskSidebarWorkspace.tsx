...
const TaskSidebarWorkspace: React.FC<TaskSidebarWorkspaceProps> = (props) => {
  ...
  const dropdownWorkspaces = availableWorkspaces.filter(
    (workspace) => !!workspace.name?.trim(),
  );

  const currentLinkedWorkspaceName =
    getFieldMeta<WorkspaceLink>(name).value?.workspace;
  const currentLinkedWorkspaceSelectable =
    currentLinkedWorkspaceName &&
    dropdownWorkspaces.some(
      (resource) => resource.name === currentLinkedWorkspaceName,
    );
